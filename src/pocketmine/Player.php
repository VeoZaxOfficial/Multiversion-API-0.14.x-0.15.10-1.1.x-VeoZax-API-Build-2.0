 ...
 */
	...
	private const INCOMING_PACKET_BATCH_PER_TICK = 20; 
	...
	/**
	 * True only while the current call stack originated from a real
	 * incoming client packet (TextPacket/CommandRequestPacket/
	 * CommandStepPacket handled in PlayerNetworkSessionAdapter), as
	 * opposed to another plugin (e.g. an NPC plugin) calling chat() or
	 * Server::dispatchCommand() programmatically on this player's behalf.
	 * Used by Guardian to distinguish a player directly typing a command
	 * from a command triggered on their behalf by something else -- see
	 * SimpleCommandMap::dispatch().
	 */
	private $isDirectClientInput = false;
	...
	public function setViewDistance(int $distance, bool $sendConfirmation = true){
		if(!$this->constructed){
			return;
		}
		$this->viewDistance = $this->server->getAllowedViewDistance($distance);
		if($this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_70){
		    $legacySpawnRadius = 2; 
		    $this->spawnThreshold = (int) (max(1, min($this->viewDistance, $legacySpawnRadius)) ** 2 * M_PI);
		    $this->chunksPerTick = 1; 
		}else{
		    $this->spawnThreshold = (int) (min($this->viewDistance, $this->server->getProperty("chunk-sending.spawn-threshold", 6)) ** 2 * M_PI);
		}
	...
	public function __construct(SourceInterface $interface, string $ip, int $port, int $raknetProtocol){
		...
		$this->protocol = ProtocolInfo::PROTOCOL_81;
		$this->loaderId = Level::generateChunkLoaderId($this);
		$this->chunksPerTick = (int) $this->server->getProperty("chunk-sending.per-tick", 4);
		$this->spawnThreshold = (int) (($this->server->getProperty("chunk-sending.spawn-threshold", 6) ** 2) * M_PI);
		$this->gamemode = $this->server->getGamemode();
		$this->setLevel($this->server->getDefaultLevel());
		$this->boundingBox = new AxisAlignedBB(0, 0, 0, 0, 0, 0);
		$this->creationTime = microtime(true);
		$this->allowMovementCheats = true; 
	...
	protected function switchLevel(Level $targetLevel) : bool{
		$oldLevel = $this->level;
		$isLegacy = $this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_70;
		if($isLegacy){
			$this->server->getLogger()->debug(
				"[WorldSwitch] " . $this->getName() . " from=" . ($oldLevel !== null ? $oldLevel->getName() : "null")
				. " to=" . $targetLevel->getName()
				. " oldDim=" . ($oldLevel !== null ? $oldLevel->getDimensionId() : "n/a")
				. " newDim=" . $targetLevel->getDimensionId()
			);
		}
		if(parent::switchLevel($targetLevel)){
			...
			if($isLegacy){
				$this->server->getLogger()->debug(
					"[WorldSwitch] " . $this->getName() . " switchLevel() completed successfully, now in " . $this->level->getName()
				);
			}
			return true;
		}
	...
	public function finishLegacyChunkSend(int $x, int $z, string $legacyBatchBuffer) : void{
		if(!$this->isConnected()){
			return;
		}
		$this->sendLegacyBatch($legacyBatchBuffer, false);
		$this->usedChunks[Level::chunkHash($x, $z)] = true;
		$this->legacyChunksSentCount = ($this->legacyChunksSentCount ?? 0) + 1;
		if($this->spawned){
	...
	public function finishP84ChunkSend(int $x, int $z, string $p84BatchBuffer) : void{
		if(!$this->isConnected()){
			return;
		}
		$this->sendP84Batch($p84BatchBuffer, false);
		$this->usedChunks[Level::chunkHash($x, $z)] = true;
		$this->legacyChunksSentCount = ($this->legacyChunksSentCount ?? 0) + 1;
		if($this->spawned){
			foreach($this->level->getChunkEntities($x, $z) as $entity){
	...
	public function sendChunk(int $x, int $z, BatchPacket $payload){
		...
		if($this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_70){
		    $chunk = $this->level->getChunk($x, $z, false);
		    if($chunk === null) return;
		    $this->legacyChunksDispatchedCount = ($this->legacyChunksDispatchedCount ?? 0) + 1;
		    $this->server->getAsyncPool()->submitTask(
		        new \pocketmine\network\mcpe\LegacyChunkRequestTask($this->level, $this, $x, $z, $chunk)
		    );
		    return;
		}
		if($this->getOriginalProtocol() >= ProtocolInfo::PROTOCOL_81 && $this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_84){
		    $chunk = $this->level->getChunk($x, $z, false);
		    if($chunk === null){
		        return;
		    }
		    $this->legacyChunksDispatchedCount = ($this->legacyChunksDispatchedCount ?? 0) + 1;
		    $this->server->getAsyncPool()->submitTask(
		        new \pocketmine\network\mcpe\P84ChunkRequestTask($this->level, $this, $x, $z, $chunk)
		    );
		    return;
		}
	...
	public function doFirstSpawn(){
		...
		if($this->server->getUpdater()->hasUpdate() and $this->hasPermission(Server::BROADCAST_CHANNEL_ADMINISTRATIVE) and true){ 
	...
	public function hasValidSpawnPosition() : bool{
		return $this->spawnPositionIsExplicit and $this->spawnPosition !== null and $this->spawnPosition->isValid();
	}
	public function setSpawn(Vector3 $pos){
		...
		$this->spawnPositionIsExplicit = true;
    ...
    public static function gamemodeToNetworkId(int $gamemode, int $playerProtocol) : int{
		return self::getClientFriendlyGamemode($gamemode);
	}
	...
	public function setGamemode(int $gm, bool $client = false) : bool{
		...
		if(!$client){ 
			...
			$this->sendMessage($this->server->getLanguage()->translateString("commands.gamemode.success.self", [Server::getGamemodeString($gm)]));
		}
		$this->inventory->sendContents($this);
    ...
    public function syncAbilities(){
	    	...
	    	$pk->entityUniqueId = $this->getId();
			    ...
			    $pk->commandPermission = AdventureSettingsPacket::PERMISSION_HOST; 
			    $pk->playerPermission = AdventureSettingsPacket::PERMISSION_HOST; 
			}
	    	$this->dataPacket($pk);
	...
	/**
	 * Applies a knockback/motion impulse to the affected player via
	 * SetActorMotionPacket, sent to themselves ONLY (not broadcast to
	 * viewers) - unlike Entity::broadcastMotion(), which sends it to
	 * everyone because non-player entities have no client-side movement
	 * of their own to kick off.
	 *
	 * A Player is client-authoritative: once this packet reaches the
	 * affected player's client, their client integrates the velocity
	 * itself and reports its new position back every tick. Those position
	 * updates are already relayed to every other viewer through the normal
	 * movement sync (processMostRecentMovements() -> broadcastMovement()),
	 * exactly like ordinary walking.
	 *
	 * Also broadcasting SetActorMotionPacket to viewers (as an earlier
	 * version of this method did) fed the same big velocity impulse
	 * straight into their client's animation controller, which drives leg-
	 * swing amplitude off raw velocity magnitude - producing the
	 * exaggerated, wide leg-swing seen on knocked-back players. Relying on
	 * the normal per-tick position sync instead avoids that: viewers just
	 * see the resulting movement play out smoothly, with regular walk
	 * animation, while the affected player still gets the correct
	 * knockback distance from the real velocity impulse.
	 */
	...
	public function checkNetwork(){
		...
		$this->checkPendingLegacyTeleport();
	...
	protected function initHumanData() : void{
		$version = ProtocolInfo::getVersionLabel($this->getOriginalProtocol());
		$this->setNameTag(TextFormat::DARK_GRAY . "[" . TextFormat::GRAY . $version . TextFormat::DARK_GRAY . "] " . TextFormat::GOLD . $this->username);
	}
	...
	public function handleLegacyLogin(string $rawBuffer) : bool {
	    if($this->loggedIn){
	        return false;
	    }
	    try{
	        $pkt = LegacyLoginPacket::fromBuffer($rawBuffer);
	    }catch(\Throwable $e){
	        $this->close("", "Invalid legacy login packet: " . $e->getMessage());
	        return false;
	    }
	    if(!self::isValidUserName($pkt->username)){
	        $this->close("", "disconnectionScreen.invalidName");
	        return true;
	    }
	    $this->originalProtocol = $pkt->protocol; 
	    $this->protocol         = ProtocolConvertor::convertProtocol($pkt->protocol); 
	    $this->chunkProtocol    = ProtocolConvertor::convertChunkProtocol($this->protocol);
	    $this->craftingProtocol = ProtocolConvertor::convertCraftingProtocol($this->protocol);
	    ...
	    $this->username    = TextFormat::clean($pkt->username);
	    $this->displayName = $this->username;
	    $this->iusername   = strtolower($this->username);
	    try{
	        $this->uuid = UUID::fromString($pkt->clientUUID);
	    }catch(\Throwable $e){
	        $this->uuid = UUID::fromString(md5($this->username));
	    }
	    $this->rawUUID = $this->uuid->toBinary();
	    $this->randomClientId = $pkt->clientId;
	    $this->enableCompression = false;
	    foreach($this->server->getLoggedInPlayers() as $p){
	        if($p !== $this && ($p->iusername === $this->iusername || $this->uuid->equals($p->getUniqueId()))){
	    ...
	    $this->playedBefore = ($this->getLastPlayed() - $this->getFirstPlayed()) > 1;
	    ...
	    $skinData = $pkt->skin;
	    $skinId   = ($pkt->skinName !== '') ? $pkt->skinName : 'Standard_Custom';
	    $accepted = Skin::ACCEPTED_SKIN_SIZES;
	    sort($accepted);
	    $skinLen = strlen($skinData);
	    foreach($accepted as $size){
	        if($skinLen <= $size){
	            $skinData = str_pad($skinData, $size, "\x00");
	            break;
	        }
	    }
	    if(!in_array(strlen($skinData), $accepted, true)){
	        $skinData = substr($skinData, 0, end($accepted));
	    }
	    $skin = new Skin($skinId, $skinData);
	    if($skin->isValid()){
	        $this->setSkin($skin);
	    }else{
	        $this->setSkin(new Skin('Standard_Custom', str_repeat("\x00", 64 * 32 * 4)));
	    }
	...
	public function onVerifyCompleted(LoginPacket $packet, ?string $error, bool $signedByMojang) : void{
		...
		if($error !== null){
            ...
            if(!$this->xboxAuthenticated){
		    	if($this->server->requiresAuthentication() and $this->kick("disconnectionScreen.notAuthenticated", false)){ 
			    	return;
		    	}
		...
		$isLegacyProtocolTier = $originalProtocol >= ProtocolInfo::PROTOCOL_41 && $originalProtocol <= ProtocolInfo::PROTOCOL_84;
		if(EncryptionContext::$ENABLED && !$isLegacyProtocolTier){
	...
	protected function completeLoginSequence(){
		...
		if($this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_70 && $this->viewDistance === -1){
		    $this->setViewDistance($this->server->getViewDistance(), false);
		}
		...
		if(!$this->hasValidSpawnPosition()){
			if($this->namedtag->getByte("SpawnForced", 0) === 1
				&& ($level = $this->server->getLevelByName($this->namedtag->getString("SpawnLevel", ""))) instanceof Level){
		...
		if($this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_70){
		    $pos = $this->getOffsetPosition($this);
		    $legacyPk = new LegacyStartGamePacket();
		    $legacyPk->seed      = -1;
		    $legacyPk->dimension = $this->level->getDimensionId();
		    $legacyPk->generator = 1;
		    $legacyPk->gamemode  = $this->gamemode & 0x01;
		    $legacyPk->eid       = 0;
		    $legacyPk->spawnX   = $spawnPosition->getFloorX();
		    $legacyPk->spawnY   = $spawnPosition->getFloorY();
		    $legacyPk->spawnZ   = $spawnPosition->getFloorZ();
		    $legacyPk->x        = (float) $pos->x;
		    $legacyPk->y        = (float) $pos->y;
		    $legacyPk->z        = (float) $pos->z;
		    $this->dataPacket($legacyPk);
		    $timePk = new LegacySetTimePacket();
		    $timePk->time    = $this->level->getTime();
		    $timePk->started = true;
		    $this->dataPacket($timePk);
		    $spawnPk = new LegacySetSpawnPacket();
		    $spawnPk->x = $spawnPosition->getFloorX();
		    $spawnPk->y = $spawnPosition->getFloorY();
		    $spawnPk->z = $spawnPosition->getFloorZ();
		    $this->dataPacket($spawnPk);
		    $healthPk = new \pocketmine\network\mcpe\protocol\SetHealthPacket();
		    $healthPk->health = (int) $this->getHealth();
		    $this->dataPacket($healthPk);
		    $diffPk = new LegacySetDifficultyPacket();
		    $diffPk->difficulty = $this->level->getDifficulty();
		    $this->dataPacket($diffPk);
		}else{
		...
		$pk->yaw = $this->yaw;
		$pk->seed = -1;
		$pk->spawnSettings = new SpawnSettings(SpawnSettings::BIOME_TYPE_DEFAULT, "", $this->getClientFriendlyDimension($this->level->getDimensionId())); 
	...
	public function isDirectClientInput() : bool{
		return $this->isDirectClientInput;
	}
	public function setDirectClientInput(bool $value) : void{
		$this->isDirectClientInput = $value;
	}
  ...
  public function removeBlock(Vector3 $blockVector) : bool{
    ...
    public function attackEntity(Entity $target) : bool{
		...
		$swingPk = new AnimatePacket();
		$swingPk->action = AnimatePacket::ACTION_SWING_ARM;
		$swingPk->entityRuntimeId = $this->getId();
		$this->server->broadcastPacket($this->getViewers(), $swingPk);
		...
		if($this->isAlive()){
			if($heldItem->onAttackEntity($target) and $this->isSurvival()){ 
	...
	public function handleCommandStep(CommandStepPacket $packet) : bool{
        ...
        $this->setDirectClientInput(true);
        try{
            $this->chat("/" . $commandText);
        }finally{
            $this->setDirectClientInput(false);
        }
	    return true;
	}
	...
	public function updateNextPosition(Vector3 $position, float $yaw, float $headYaw, float $pitch) : bool{
		...
		if($this->pendingLegacyTeleport !== null){
			return true;
		}
		if($this->forceMoveSync !== null and $newPos->distanceSquared($this->forceMoveSync) > 1){  
	...
	public function handleEntityEvent(ActorEventPacket $packet) : bool{
		...
		switch($packet->event){
			case ActorEventPacket::PLAYER_ADD_XP_LEVELS:
				...
				break;
			case ActorEventPacket::USE_ITEM: 
			...
			case ActorEventPacket::EATING_ITEM: 
				...
				foreach($viewers as $viewer){
					...
					$pk = new ActorEventPacket();
					$pk->entityRuntimeId = $this->getId();
					if($viewer->getProtocol() < ProtocolInfo::PROTOCOL_90){
	...
	public function handleInventoryTransaction(InventoryTransactionPacket $packet) : bool{
		...
		if($packet->trData instanceof NormalTransactionData){
			$result = $this->handleNormalTransaction($packet->trData, $packet->requestId ?? 0);
		...
		switch($data->getActionType()){
            case UseItemTransactionData::ACTION_CLICK_BLOCK:
                $spamBug = ($this->lastRightClickData !== null and
                    microtime(true) - $this->lastRightClickTime < 0.1 and 
				    ...
				    $this->lastRightClickData->getClickPos()->distanceSquared($data->getClickPos()) < 0.00001 
                );
	...
	protected function broadcastMovement(bool $teleport = false) : void{
		...
		foreach($viewers as $viewer){
			if($viewer->getProtocol() < ProtocolInfo::PROTOCOL_90){
				$oldProtocolViewers[] = $viewer;
			}else{
				$newProtocolViewers[] = $viewer;
			}
		}
		if(!empty($newProtocolViewers)){
			...
			$this->server->broadcastPacket($newProtocolViewers, $pk);
		}
		if(!empty($oldProtocolViewers)){
			...
			$this->server->broadcastPacket($oldProtocolViewers, $pk);
		}
	}
	...
	public function toggleFlight(bool $flying) : bool{
	...
	public function sendDataPacket($packet, bool $needACK = false, bool $immediate = false){
		if(!$this->isConnected()){
			return false;
		}
		$isLegacyP70 = ($packet instanceof \pocketmine\network\mcpe\protocol\legacy\p70\DataPacket);
		if(!$isLegacyP70 and !$this->loggedIn and !$packet->canBeSentBeforeLogin()){
	...
	public function dataPacket($packet, bool $needACK = false){
		return $this->sendDataPacket($packet, $needACK, false);
	}
	public function directDataPacket($packet, bool $needACK = false){
		return $this->sendDataPacket($packet, $needACK, true);
	}
	public function sendLegacyBatch(string $encodedBuffer, bool $immediate = true) : void {
		if(!$this->isConnected()) return;
		$this->interface->sendLegacyBuffer($this, $encodedBuffer, $immediate);
	}
	public function sendP84Batch(string $encodedBuffer, bool $immediate = true) : void {
		if(!$this->isConnected()) return;
		$this->interface->sendP84Buffer($this, $encodedBuffer, $immediate);
	}
	...
	public function save(){
		...
		if($this->hasValidSpawnPosition()){
			...
			$this->namedtag->setByte("SpawnForced", 1);
			if(!$this->isAlive()){
				$this->namedtag->setTag(new ListTag("Pos", [
			...
			$this->namedtag->removeTag("SpawnLevel", "SpawnX", "SpawnY", "SpawnZ", "SpawnForced");
		}
	...
	public function teleport(Vector3 $pos, float $yaw = null, float $pitch = null) : bool{
		if(parent::teleport($pos, $yaw, $pitch)){
			...
			if($this->getOriginalProtocol() <= ProtocolInfo::PROTOCOL_70){
				$this->pendingLegacyTeleport = $pos->asVector3();
			}else{
	...
	protected function checkPendingLegacyTeleport() : void{
		if($this->pendingLegacyTeleport === null){
			return;
		}
		$chunkX = (int)$this->pendingLegacyTeleport->x >> 4;
		$chunkZ = (int)$this->pendingLegacyTeleport->z >> 4;
		for($x = -1; $x <= 1; ++$x){
			for($z = -1; $z <= 1; ++$z){
				$index = Level::chunkHash($chunkX + $x, $chunkZ + $z);
				if(!isset($this->usedChunks[$index]) or $this->usedChunks[$index] === false){
					return; 
				}
			}
		}
		$this->sendPosition($this, null, null, MovePlayerPacket::MODE_RESET);
		$this->forceMoveSync = $this->pendingLegacyTeleport;
		$this->pendingLegacyTeleport = null;
	}
