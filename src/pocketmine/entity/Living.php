...
use pocketmine\block\Block;use pocketmine\event\entity\EntityDamageByChildEntityEvent;use pocketmine\event\entity\EntityDamageByEntityEvent;use pocketmine\event\entity\EntityDamageEvent;use pocketmine\event\entity\EntityDeathEvent;use pocketmine\event\entity\EntityEffectAddEvent;use pocketmine\event\entity\EntityEffectRemoveEvent;use pocketmine\entity\projectile\EnderPearl;use pocketmine\entity\data\KnockBackData;use pocketmine\inventory\ArmorInventory;use pocketmine\inventory\ArmorInventoryEventProcessor;use pocketmine\item\Armor;use pocketmine\item\Consumable;use pocketmine\item\Durable;use pocketmine\item\enchantment\Enchantment;use pocketmine\item\Item;use pocketmine\math\Vector3;use pocketmine\math\VoxelRayTrace;use pocketmine\nbt\tag\ByteTag;use pocketmine\nbt\tag\CompoundTag;use pocketmine\nbt\tag\FloatTag;use pocketmine\nbt\tag\IntTag;use pocketmine\nbt\tag\ListTag;use pocketmine\network\mcpe\protocol\ActorEventPacket;use pocketmine\network\mcpe\protocol\LevelSoundEventPacket;use pocketmine\network\mcpe\protocol\MobEffectPacket;use pocketmine\Server;use pocketmine\Player;use pocketmine\timings\Timings;use pocketmine\utils\Binary;use pocketmine\utils\Color;use function abs;use function array_shift;use function atan2;use function ceil;use function count;use function floor;use function lcg_value;use function max;use function min;use function mt_getrandmax;use function mt_rand;use function sqrt;use function in_array;use const M_PI;
abstract class Living extends Entity implements Damageable{
	...
	public function removeEffect(int $effectId) : void{
		if(isset($this->effects[$effectId])){
			...
			$ev->call();
			if($ev->isCancelled()){
				if($hasExpired and !$ev->getEffect()->hasExpired()){ 
	...
	public function attack(EntityDamageEvent $source) : void{
		...
		$this->attackTime = 10; //0.5 seconds cooldown -- matches VeoZaxAPI 1.0.0's fixed classic timing
		$this->noDamageTicks = 10; //VeoZaxAPI: prevent multi-source damage spam
		if($source instanceof EntityDamageByEntityEvent){
			$e = $source->getDamager();
