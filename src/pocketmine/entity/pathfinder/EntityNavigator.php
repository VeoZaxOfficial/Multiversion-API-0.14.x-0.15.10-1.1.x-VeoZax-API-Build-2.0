...
use pocketmine\block\Block;use pocketmine\block\Lava;use pocketmine\block\Liquid;use pocketmine\block\Water;use pocketmine\entity\Mob;use pocketmine\math\Vector3;use pocketmine\timings\Timings;use function abs;use function array_unshift;use function array_values;use function count;use function floor;use function reset;use function uasort;
class EntityNavigator{
	...
	public function getNeighbors(PathPoint $tile, array &$cache, int $startY) : array{
		...
		for($index = 0; $index < count($this->neighbors); ++$index){
			...
			if(!$tb->isPassable()){
				...
				if(!$blockDown->isSolid() and !$this->mob->isSwimmer() and !($blockDown instanceof Liquid) and !$this->mob->canFly()){ 
					if($this->mob->canClimb()){
						$canClimb = false;
	...
	public function isSafeToStandAt(Vector3 $pos, bool $onlySee = false) : bool{
		if($this->isObstructed($pos) and !$onlySee){
			...
			if(($block instanceof Water and !$this->mob->isSwimmer() and !$this->mob->canFly() and $this->avoidsWater) or $block instanceof Lava){ 
				return false;
			}else{
				return true;
			}
