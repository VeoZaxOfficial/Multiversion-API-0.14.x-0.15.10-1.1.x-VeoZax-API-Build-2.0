...
use pocketmine\block\Block;use pocketmine\level\ChunkManager;use pocketmine\math\Math;use pocketmine\math\Vector3;use pocketmine\math\VectorMath;use pocketmine\utils\Random;
class Cave extends Populator {
	...
	public function populate(ChunkManager $level, $chunkX, $chunkZ, Random $random){
		...
		if($random->nextBoundedInt(8) == 1){
			$caves[0] = $random->nextRange(1, 360); 
			$caves[1] = $random->nextRange(45, 300); 
			$caves[2] = $random->nextRange(1, 5); 
			$caves[4] = $random->nextRange(1, 5); 
		...
		if($random->nextBoundedInt(5) == 1){
			$caves[0] = $random->nextRange(1, 360); 
			$caves[1] = $random->nextRange(50, 300); 
			$caves[2] = $random->nextRange(1, 5); 
			$caves[4] = $random->nextRange(1, 5); 
			$caves[3] = [false, true, true];
			if($this->biome == 24){
    ...
    public function caves(Random $random, ChunkManager $level, Vector3 $pos, $cave, $tt = false) {
        ...
        for ($u = 0; $u <= $ls; $u += $i) {
            ...
            for ($m = 0; $m <= $l; $m++) {
                $v = $level->getBlockIdAt((int) round($x + $this->chu($m * ($x2 - $x), $l)), (int) round($y + $this->chu($m * ($y2 - $y), $l)), (int) round($z + $this->chu($m * ($z2 - $z), $l)));
                if ($v != 10 and $v != 11){
					$liu = $random->nextRange(0, 200) == 100;
					$this->fdx((int) round($x + $this->chu($m * ($x2 - $x), $l)), (int) round($y + $this->chu($m * ($y2 - $y), $l)), (int) round($z + $this->chu($m * ($z2 - $z), $l)), $level, $liu);
				}
            }
            $s1 = [$s2[0], $s2[1], $s2[2]];
        }
