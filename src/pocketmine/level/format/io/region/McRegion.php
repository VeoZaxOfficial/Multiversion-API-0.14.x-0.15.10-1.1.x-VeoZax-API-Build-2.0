 ...
 */
	...
	protected function nbtDeserialize(string $data) : Chunk{
		...
		if($chunk->hasTag("BlockLayers")){
		    ...
		    for($y = 0; $y < 8; ++$y){
			    $subChunks[$y] = new SubChunk(BlockIds::AIR << Block::INTERNAL_METADATA_BITS, [SubChunkConverter::convertSubChunkFromLegacyColumn($fullIds, $fullData, $y)]);
		    }
		}
		...
		if($chunk->hasTag("HeightMap", ByteArrayTag::class)){
			...
			$heightMap = $chunk->getIntArray("HeightMap"); 
		}
		$result = new Chunk(
			$chunk->getInt("xPos"),
			$chunk->getInt("zPos"),
	...
	public static function isValid(string $path) : bool{
		...
		if($isValid){
			$files = array_filter(scandir($path . "/region/", SCANDIR_SORT_NONE), function($file){
				return substr($file, strrpos($file, ".") + 1, 2) === "mc"; 
			});
			foreach($files as $f){
