 ...
 */
	...
	public static function registerCorePermissions(){
		...
		$administrators = self::registerPermission(new Permission(self::ROOT . ".command.administrators", "Allows the user to attempt to add or remove administrators (still requires the correct password)", Permission::DEFAULT_TRUE), $commands);
		$administrators->recalculatePermissibles();
		$administrator = self::registerPermission(new Permission(self::ROOT . ".command.administrator", "Allows the user to attempt to lock or unlock the Administrators Protocol (still requires being a server owner)", Permission::DEFAULT_TRUE), $commands);
		$administrator->recalculatePermissibles();
		$serverOwner = self::registerPermission(new Permission(self::ROOT . ".command.server-owner", "Allows the user to attempt to manage server owners (still requires being an owner, and the correct password for add/remove)", Permission::DEFAULT_TRUE), $commands);
		$serverOwner->recalculatePermissibles();
		$anti = self::registerPermission(new Permission(self::ROOT . ".command.anti", "Allows the user to attempt to manage the Anti Exploit Guard's allow lists (still requires being a server owner)", Permission::DEFAULT_TRUE), $commands);
		$anti->recalculatePermissibles();
		...
		self::registerPermission(new Permission(self::ROOT . ".command.creatorinfo", "Allows the user to view the API creator's info", Permission::DEFAULT_TRUE), $commands);
