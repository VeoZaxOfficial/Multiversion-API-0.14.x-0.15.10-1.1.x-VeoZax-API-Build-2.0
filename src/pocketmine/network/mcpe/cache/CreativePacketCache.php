...
use pocketmine\item\Durable;use pocketmine\item\Item;use pocketmine\network\mcpe\protocol\CreativeContentPacket;use pocketmine\network\mcpe\protocol\ProtocolInfo;use pocketmine\network\mcpe\protocol\types\inventory\CreativeItemEntry;use pocketmine\utils\SingletonTrait;use function file_get_contents;use function json_decode;use function array_diff;use function scandir;use function array_keys;use function krsort;use const DIRECTORY_SEPARATOR;use const pocketmine\RESOURCE_PATH;
final class CreativePacketCache{
    ...
    private static function make() : self{
        ...
        foreach(array_diff(scandir($itemsDirectory), ["..", "."]) as $protocol){
            ...
            foreach($itemsProtocol as $itemJson){
                ...
                $groupId = isset($itemJson["groupId"]) ? (int) $itemJson["groupId"] : ( (CreativeContentPacket::CREATIVE_GROUP_NONE) );
    ...
    public function addItem(Item $item, ?int $groupId = null, ?int $protocolVersion = null) : void{
        $addItemClosure = function(int $protocol, Item $item, ?int $groupId) : void {
            ...
            $groupId = $groupId ?? ( (CreativeContentPacket::CREATIVE_GROUP_NONE) );
