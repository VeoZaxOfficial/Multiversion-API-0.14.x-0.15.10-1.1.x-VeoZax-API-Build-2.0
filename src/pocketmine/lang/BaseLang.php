...
use GlobalLogger;use function array_filter;use function array_map;use function file_exists;use function is_dir;use function ord;use function parse_ini_file;use function scandir;use function str_replace;use function strlen;use function strpos;use function strtolower;use function substr;use const INI_SCANNER_RAW;use const pocketmine\PATH;use const SCANDIR_SORT_NONE;
class BaseLang{
	...
	protected function parseTranslation(string $text, string $onlyPrefix = null) : string{
		...
		for($i = 0; $i < $len; ++$i){
			...
			if($replaceString !== null){
				$ord = ord($c);
				if(
					($ord >= 0x30 and $ord <= 0x39) 
					or ($ord >= 0x41 and $ord <= 0x5a) 
					or ($ord >= 0x61 and $ord <= 0x7a) or 
					$c === "." or $c === "-"
				){
					$replaceString .= $c;
				}else{
