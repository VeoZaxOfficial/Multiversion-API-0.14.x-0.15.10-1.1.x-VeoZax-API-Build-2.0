...
use pocketmine\command\Command;use pocketmine\command\CommandSender;use pocketmine\command\utils\InvalidCommandSyntaxException;use pocketmine\lang\TranslationContainer;use pocketmine\math\Vector3;use pocketmine\Player;use pocketmine\utils\TextFormat;use function array_filter;use function array_values;use function count;use function round;
class TeleportCommand extends VanillaCommand{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args){
		...
		if(count($args) < 3){
			...
			$message = $sender->getServer()->getLanguage()->translateString("commands.tp.success.coordinates", [$target->getName(), round($x, 2), round($y, 2), round($z, 2)]);
