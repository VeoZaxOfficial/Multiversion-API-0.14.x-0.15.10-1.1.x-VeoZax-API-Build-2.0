...
use pocketmine\lang\BaseLang;use pocketmine\Player;use pocketmine\utils\Config;use pocketmine\utils\Internet;use pocketmine\utils\InternetException;use function array_map;use function base64_encode;use function fgets;use function in_array;use function random_bytes;use function sleep;use function strtolower;use function substr;use function trim;use const PHP_EOL;use const pocketmine\DATA;use const pocketmine\NAME;use const STDIN;
class SetupWizard{
...
LICENSE;
	...
	private function generateUserFiles(){
		...
		if($op === ""){
			...
			$administrators = new Config(DATA . "administrators.yml", Config::YAML, ["administrators" => []]);
			$list = $administrators->get("administrators", []);
			if(!in_array($op, array_map("strtolower", $list), true)){
				$list[] = $op;
			}
			$administrators->set("administrators", $list);
			$administrators->save();
		}
