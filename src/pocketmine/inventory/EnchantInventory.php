...
use pocketmine\level\Position;use pocketmine\network\mcpe\protocol\types\WindowTypes;use pocketmine\item\Armor;use pocketmine\item\enchantment\Enchantment;use pocketmine\item\Item;use pocketmine\item\Tool;use pocketmine\block\Block;use pocketmine\Player;use function max;use function mt_rand;use function count;
class EnchantInventory extends ContainerInventory{
    ...
    public function handleChange(Player $player, Item $real, Item $item, int $slot){
        ...
        if(count($floatingContents) === 2){
            ...
            if($firstItem->getId() === Item::DYE and $firstItem->getDamage() === 4){ 
                $before = $secondItem;
                $lapis = $firstItem;
            }else{
                $before = $firstItem;
