...
class MemoryManager{
	...
	private function init(){
		...
		$this->lowMemChunkRadiusOverride = (int) $this->server->getProperty("memory.max-chunks.trigger-limit", 28);
		$this->lowMemChunkGC = (bool) $this->server->getProperty("memory.max-chunks.trigger-chunk-collect", true);
		$this->lowMemDisableChunkCache = (bool) $this->server->getProperty("memory.world-caches.disable-chunk-cache", false);
		$this->lowMemClearWorldCache = (bool) $this->server->getProperty("memory.world-caches.low-memory-trigger", true);
		$this->dumpWorkers = true; 
		gc_enable();
	}
