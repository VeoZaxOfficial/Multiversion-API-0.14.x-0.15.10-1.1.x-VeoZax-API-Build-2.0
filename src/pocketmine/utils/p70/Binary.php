		...
		foreach($data as $bottom => $d){
			...
			switch($d[0]){
				...
				case Entity::DATA_TYPE_SLOT:
					$m .= self::writeLShort($d[1][0] ?? 0);
					$m .= self::writeByte($d[1][1] ?? 0);
					$m .= self::writeLShort($d[1][2] ?? 0);
					break;
				case Entity::DATA_TYPE_POS:
					...
					if(is_object($d[1]) && method_exists($d[1], 'getFloorX')){
					    $m .= self::writeLInt((int)$d[1]->getFloorX());
					    $m .= self::writeLInt((int)$d[1]->getFloorY());
					    $m .= self::writeLInt((int)$d[1]->getFloorZ());
					}elseif(is_array($d[1])){
					    $m .= self::writeLInt($d[1][0] ?? 0);
					    $m .= self::writeLInt($d[1][1] ?? 0);
					    $m .= self::writeLInt($d[1][2] ?? 0);
					}else{
					    $m = substr($m, 0, -1); 
					}
					break;
