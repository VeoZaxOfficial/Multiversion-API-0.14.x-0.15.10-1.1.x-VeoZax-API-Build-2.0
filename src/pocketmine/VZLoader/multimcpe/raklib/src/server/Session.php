 ...
 */
	...
	private $gracePeriodSeconds = 2.0;
	private $graceStartTime = null;
	private $ACKQueue = [];
	private $NACKQueue = [];
	...
	public function setGracePeriod(float $seconds) : void{
		$this->gracePeriodSeconds = max(0.0, $seconds);
	}
	...
	public function update(float $time) : void{
		if(!$this->isActive and ($this->lastUpdate + 10) < $time){
			if($this->graceStartTime === null){
				$this->graceStartTime = $time;
				return;
			}
			if(($time - $this->graceStartTime) < $this->gracePeriodSeconds){
				return;
			}
